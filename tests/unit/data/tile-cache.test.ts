...
const key = (trackId: string, chrom: string, binSize: number, binIndex: number) =>
  `${trackId}:${chrom}:${binSize}:${binIndex}`;

describe('parseTileKey / formatTileKey', () => {
  it('round-trips a valid key', () => {
    const k = 'tr1:chr20:1024:42';
    const p = parseTileKey(k);
    expect(p).toEqual({ trackId: 'tr1', chrom: 'chr20', binSize: 1024, binIndex: 42 });
    expect(formatTileKey(p!)).toBe(k);
  });

  it.each([
    'tr1:chr20:1024',
    'tr1:chr20:1024:42:extra',
    'tr1:chr20:9999:42',  // not in BIN_SIZES
    'tr1:chr20:1024:-1',
    'tr1:chr20:1024:abc',
    ':chr20:1024:42',
    'tr1::1024:42',
...
describe('TileCache basic ops', () => {
  ...
  it('snapshot is a fresh map each call', () => {
    const c = createTileCache();
    c.put(key('t', 'chr1', 1024, 0), pending);
    const a = c.snapshot();
    const b = c.snapshot();
    expect(a).not.toBe(b);
    expect([...a]).toEqual([...b]);
...
describe('eviction', () => {
  it('LRU when no viewport set', () => {
    ...
    c.put(key('t', 'chr1', 1024, 0), pending);
    c.put(key('t', 'chr1', 1024, 1), pending);
    c.put(key('t', 'chr1', 1024, 2), pending);
    c.put(key('t', 'chr1', 1024, 3), pending); // forces eviction

    expect(c.size()).toBe(3);
    expect(c.evictionCount()).toBe(1);
    // bin 0 is oldest -> evicted
    expect(c.has(key('t', 'chr1', 1024, 0))).toBe(false);
    expect(c.has(key('t', 'chr1', 1024, 3))).toBe(true);
  });

  it('viewport-distance dominates LRU', () => {
    let t = 1000;
    const c = createTileCache({ capacity: 2, now: () => t++ });
    // Viewport centered around bin 100 on chr1.
    ...
    c.put(key('t', 'chr1', 1024, 0), pending);     // far
    c.put(key('t', 'chr1', 1024, 100), pending);   // near viewport
    c.put(key('t', 'chr1', 1024, 99), pending);    // near, newer

    expect(c.size()).toBe(2);
    expect(c.has(key('t', 'chr1', 1024, 0))).toBe(false);   // far -> evicted
    expect(c.has(key('t', 'chr1', 1024, 100))).toBe(true);
    expect(c.has(key('t', 'chr1', 1024, 99))).toBe(true);
  });
  ...
  it('different chrom is more evictable than same-chrom', () => {
    ...
    c.put(key('t', 'chr20', 1024, 0), pending); // off-viewport chrom
    c.put(key('t', 'chr1', 1024, 1000), pending); // on-chrom but far
    expect(c.size()).toBe(1);
    expect(c.has(key('t', 'chr20', 1024, 0))).toBe(false);
    expect(c.has(key('t', 'chr1', 1024, 1000))).toBe(true);
  });

  it('preserves error and ready entries equally — eviction is status-blind', () => {
    let t = 1000;
    const c = createTileCache({ capacity: 2, now: () => t++ });
    c.put(key('t', 'chr1', 1024, 0), errored('x'));
    c.put(key('t', 'chr1', 1024, 1), pending);
    c.put(key('t', 'chr1', 1024, 2), pending); // evicts the oldest = errored
    expect(c.has(key('t', 'chr1', 1024, 0))).toBe(false);
  });
});
